...
if(isset($_GET['code_title']) && !empty($_GET['code_title']))
{
    $code_title = $_GET['code_title'];
    $code_idx =$_GET['code_idx'];
    $stmt_edit = $con->prepare('SELECT * FROM nakaracubae_code WHERE idx=:idx');
    $stmt_edit->execute(array(':idx'=>$code_idx));
    $edit_row = $stmt_edit->fetch(PDO::FETCH_ASSOC);
    extract($edit_row);
}
    ...
    if( ($_SERVER['REQUEST_METHOD'] == 'POST') && isset($_POST['submit']))
{
    ...
    $usercontent=$_POST['content'];
            ...
            try { 
                $stmt = $con->prepare('select * from nakaracubae_board where title=:title idx=:idx');
                            ...
                            <main>
                                <div class="container-fluid px-4">
                                    ...
                                    <li class="breadcrumb-item active" name="<?php echo $name ?>" value="name">☑️ 작성자:<?php echo $name ?></li>
                                        ...
                                        <li class="breadcrumb-item">✅ 언어: <?php echo $language; ?></li>
                                        </li>
                                    </ol>
