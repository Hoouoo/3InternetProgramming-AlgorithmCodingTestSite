...
if(isset($_GET['user_id']) && !empty($_GET['user_id']))
{
        ...
        if( ($_SERVER['REQUEST_METHOD'] == 'POST') && isset($_POST['submit']))
	{
       ...
       $write_language=$_POST['language'];
       $date = date("Y-m-d");



                try { 
                    $stmt = $con->prepare('select * from nakaracubae_code where name=:name');
                   ...
                   try{
			$stmt = $con->prepare('INSERT INTO nakaracubae_code (name, title, content, language, date) VALUES(:name, :title, :content, :language, :date)');
			$stmt->bindParam(':name',$name);
            $stmt->bindParam(':title',$title);
            $stmt->bindParam(':content',$write_content);
            $stmt->bindParam(':language',$write_language);
            $stmt->bindParam(':date',$date);

			if($stmt->execute())
			{
				$successMSG = "성공.";
				header("refresh:1;codeboard.php?user_id=".$id);
			}
			else
			{
				$errMSG = "사용자 추가 에러";
                                    ...
                                    <li class="breadcrumb-item active" name="<?php echo $name ?>" value="name">☑️ 작성자:<?php echo $name ?></li>
                                        ...
                                        <div class="col-md-5">
                                            <input
                                                type="text"
                                                class="form-control"
                                                placeholder="사용한 언어를 작성하시오."
                                                name="language"
                                                autocomplete="off"
                                                name="board_subject"
                                                value="">
                                                <input type="hidden" name="__autocomplete_fix_language" value="language"/>
                                        </div>
                                    </div>
                                        <div class="form-floating mb-3">
                                            <label class="col-md-2 control-label" for="board_subject"></label>
